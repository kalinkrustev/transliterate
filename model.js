...
module.exports.createModel = function createModel(
    inputVocabSize, outputVocabSize, inputLength, outputLength, embeddingDims = 2, lstmUnits = 32) {
    ...
    let attention = tf.layers.dot({
        axes: [2, 2]
    }).apply([decoder, encoder]);
 ...
 */
module.exports.runSeq2SeqInference = async function runSeq2SeqInference(
    model, inputStr, getAttention = false) {
    return tf.tidy(() => {
        const encoderInput = translit.encodeInputStrings([inputStr]);
        const decoderInput = tf.buffer([1, translit.OUTPUT_LENGTH]);
        decoderInput.set(translit.START_CODE, 0, 0);

        for (let i = 1; i < translit.OUTPUT_LENGTH; ++i) {
        ...
        decoderFinalOutput =
            decoderFinalOutput.argMax(2).dataSync()[translit.OUTPUT_LENGTH - 1];

        for (let i = 1; i < decoderInput.shape[1]; ++i) {
            if (translit.OUTPUT_VOCAB[decoderInput.get(0, i)] === '\n') break;
            output.outputStr += translit.OUTPUT_VOCAB[decoderInput.get(0, i)];
        }
        output.outputStr += translit.OUTPUT_VOCAB[decoderFinalOutput];
        return output;
    });
};
